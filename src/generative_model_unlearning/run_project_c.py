...
def main():
    ...
    for task_id in range(NUM_TASKS):
        ...
        # 💡 FIX: Create a fresh optimizer and scheduler for each task
    ...
    # ... (بقیه کد main بدون تغییر باقی می‌ماند و باید درست کار کند)
    ...
    print(f"Average Accuracy: {np.mean(before_avg_acc):.2f}%")
        ...
        for _ in pbar:
            ...
            if re_batch is not None:
                ...
                for t_id in range(NUM_TASKS):
                    if t_id == task_to_forget: continue
                    mask = (re_task_ids == t_id)
                    if mask.sum() > 1:
                        re_outputs = solver_model(re_images[mask], task_id=t_id)
                        loss += criterion(re_outputs, re_labels[mask])
